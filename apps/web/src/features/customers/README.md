...
Clientes usa `EntityTypeChoice` antes da criação, `EntityStepForm` na criação/edição e `EntityDetail` para a visualização por cards, em `features/parties`. Esses componentes foram extraídos do fluxo de Clientes para atender também Fornecedores, com etapas e validações próprias de cada módulo. A barra `StickyFormActions` é reutilizável em outros formulários. Os dados gerais, fiscais e financeiros pertencem ao mesmo rascunho e somente são gravados na última etapa.
